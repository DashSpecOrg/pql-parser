...
describe("validator.ts", () => {
    ...
    test("valid aggregation query with GROUP BY", () => {
        const query = new Parser(new Lexer("PLOT BAR(category, SUM(value)) FROM t GROUP BY category")).parse();
        const result = validator.validate(query);
        expect(result.valid).toBe(true);
    });

    test("aggregation without GROUP BY is invalid", () => {
        const query = new Parser(new Lexer("PLOT BAR(category, SUM(value)) FROM t")).parse();
        const result = validator.validate(query);
    ...
    test("HAVING without GROUP BY is invalid", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t HAVING COUNT() > 5")).parse();
        const result = validator.validate(query);
        expect(result.valid).toBe(false);
    ...
    test("valid HAVING with GROUP BY", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, COUNT()) FROM t GROUP BY x HAVING COUNT() > 5")).parse();
        const result = validator.validate(query);
        expect(result.valid).toBe(true);
    });

    test("negative LIMIT is invalid", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t LIMIT 10")).parse();
    ...
    test("HISTOGRAM with aggregation is invalid", () => {
        const query = new Parser(new Lexer("PLOT HISTOGRAM(SUM(value)) FROM t GROUP BY x")).parse();
        const result = validator.validate(query);
        expect(result.valid).toBe(false);
    });

    test("HISTOGRAM with negative bins is invalid", () => {
        const query = new Parser(new Lexer("PLOT HISTOGRAM(value, 10) FROM t")).parse();
