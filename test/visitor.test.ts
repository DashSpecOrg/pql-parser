...
describe("visitor.ts", () => {
    describe("ColumnCollector", () => {
        ...
        test("collects columns from ORDER BY", () => {
            const query = new Parser(new Lexer("PLOT LINE(x, y) ORDER BY x")).parse();
            const columns = collector.visitQuery(query);
            expect(columns).toContain("x");
        });

        test("collects columns from HAVING clause", () => {
            const query = new Parser(new Lexer("PLOT BAR(cat, SUM(val)) GROUP BY cat HAVING AVG(price) > 10")).parse();
            const columns = collector.visitQuery(query);
            expect(columns).toContain("price");
        });

        test("collects columns from BETWEEN condition", () => {
            const query = new Parser(new Lexer("PLOT BAR(x, y) WHERE age BETWEEN 18 AND 65")).parse();
            const columns = collector.visitQuery(query);
            expect(columns).toContain("age");
        });

        test("collects columns from IN condition", () => {
            const query = new Parser(new Lexer("PLOT BAR(x, y) WHERE status IN ('a', 'b')")).parse();
            const columns = collector.visitQuery(query);
        ...
        test("collects columns from HEATMAP", () => {
            const query = new Parser(new Lexer("PLOT HEATMAP(a, b, c)")).parse();
            const columns = collector.visitQuery(query);
        ...
        test("collects columns from NOT condition", () => {
            const query = new Parser(new Lexer("PLOT BAR(x, y) WHERE NOT active > 0")).parse();
            const columns = collector.visitQuery(query);
    ...
    describe("AggregationChecker", () => {
        ...
        test("detects aggregation in query", () => {
            const query = new Parser(new Lexer("PLOT BAR(x, SUM(y)) GROUP BY x")).parse();
            expect(checker.visitQuery(query)).toBe(true);
        });

        test("detects COUNT aggregation", () => {
            const query = new Parser(new Lexer("PLOT BAR(x, COUNT()) GROUP BY x")).parse();
            expect(checker.visitQuery(query)).toBe(true);
        });

        test("detects aggregation in HEATMAP", () => {
            const query = new Parser(new Lexer("PLOT HEATMAP(x, y, AVG(z)) GROUP BY x")).parse();
            expect(checker.visitQuery(query)).toBe(true);
        });
    });
});
