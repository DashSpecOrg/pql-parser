...
describe("pretty-printer.ts", () => {
    test("prints simple BAR query", () => {
        ...
        expect(result).toBe("PLOT BAR(x, y) FROM t");
    });

    test("prints query with WHERE clause", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t WHERE z > 0")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toBe("PLOT BAR(x, y) FROM t WHERE z > 0");
    });

    test("prints query with GROUP BY", () => {
        const query = new Parser(new Lexer("PLOT BAR(cat, SUM(val)) FROM t GROUP BY cat")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toBe("PLOT BAR(cat, SUM(val)) FROM t GROUP BY cat");
    });

    test("prints query with HAVING", () => {
        const query = new Parser(new Lexer("PLOT BAR(cat, SUM(val)) FROM t GROUP BY cat HAVING SUM(val) > 100")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toContain("HAVING SUM(val) > 100");
    });

    test("prints query with ORDER BY", () => {
        const query = new Parser(new Lexer("PLOT LINE(x, y) FROM t ORDER BY x DESC")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toContain("ORDER BY x DESC");
    });

    test("prints query with LIMIT and OFFSET", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t LIMIT 10 OFFSET 5")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toContain("LIMIT 10 OFFSET 5");
    });

    test("prints BETWEEN condition", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t WHERE z BETWEEN 1 AND 10")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toContain("z BETWEEN 1 AND 10");
    });

    test("prints IN condition", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t WHERE cat IN ('a', 'b')")).parse();
        const result = prettyPrint(query, { newlines: false });
    ...
    test("prints NOT condition", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t WHERE NOT z > 0")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toContain("NOT z > 0");
    });

    test("prints PIE plot", () => {
        const query = new Parser(new Lexer("PLOT PIE(cat, val) FROM t")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toBe("PLOT PIE(cat, val) FROM t");
    });

    test("prints HISTOGRAM plot with bins", () => {
        const query = new Parser(new Lexer("PLOT HISTOGRAM(values, 10) FROM t")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toBe("PLOT HISTOGRAM(values, 10) FROM t");
    });

    test("prints HEATMAP plot", () => {
        const query = new Parser(new Lexer("PLOT HEATMAP(x, y, z) FROM t")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toBe("PLOT HEATMAP(x, y, z) FROM t");
    });
    ...
    test("prints with newlines", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t WHERE z > 0 LIMIT 10")).parse();
        const result = prettyPrint(query, { newlines: true });
        expect(result).toContain("\n");
    });

    test("escapes special identifiers", () => {
        const query = new Parser(new Lexer("PLOT BAR(`my column`, y) FROM t")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toContain("`my column`");
    });

    test("prints column alias", () => {
        const query = new Parser(new Lexer("PLOT BAR(xcol AS x, ycol AS y) FROM t")).parse();
        const result = prettyPrint(query, { newlines: false });
    ...
    test("prints NULL value", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM t WHERE z = NULL")).parse();
        const result = prettyPrint(query, { newlines: false });
    ...
    test("prints FROM with escaped table name", () => {
        const query = new Parser(new Lexer("PLOT BAR(x, y) FROM `my table`")).parse();
        const result = prettyPrint(query, { newlines: false });
        expect(result).toBe("PLOT BAR(x, y) FROM `my table`");
    });

    test("round-trip parsing", () => {
        const original = "PLOT BAR(category, SUM(value)) FROM sales WHERE status = 'active' GROUP BY category HAVING SUM(value) > 100 ORDER BY category ASC LIMIT 10";
        ...
        expect(reparsed.fromClause.table).toBe(query.fromClause.table);
